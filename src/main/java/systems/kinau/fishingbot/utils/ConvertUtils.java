...
public class ConvertUtils {
    ...
    public static Object convert(String value, Class type, Type genericType) {
        ...
        } else if(type.isAssignableFrom(AuthMode.class)) {
            try {
                return AuthMode.valueOf(value);
            } catch (IllegalArgumentException ex) {
                FishingBot.getI18n().warning("config-unknown-auth-type", value);
                ex.printStackTrace();
                return AuthMode.Online;
            }
        } else if(type.isAssignableFrom(AutoRegisterMode.class)) {
            try {
                return AutoRegisterMode.valueOf(value);
            } catch (IllegalArgumentException ex) {
                FishingBot.getI18n().warning("config-unknown-autoregister-type", value);
                ex.printStackTrace();
                return AutoRegisterMode.TwoPassword;
            }
